...
class LGM(nn.Module):
    ...
    def prepare_default_rays_24(self, device, elevation=0):
        from kiui.cam import orbit_camera
        from core.utils import get_rays

        # 注意默认的角度定义方式
        cam_poses = np.stack([
            orbit_camera(elevation, azimuth, radius=self.opt.cam_radius)
            for azimuth in np.arange(0, 360, 15)
        ], axis=0) # [24, 4, 4]
        cam_poses = torch.from_numpy(cam_poses)
    ...
    def forward_gaussians(self, images):
        ...
        images = images.view(B*V, C, H, W) # 输入图像
    ...
    def forward(self, data, step_ratio=1):
        ...
        if self.opt.lambda_lpips > 0: # LPIPS 损失计算
