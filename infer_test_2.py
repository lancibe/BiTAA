...
from core.gs_filter import compute_3D_filter, apply_3D_filter, save_fused_ply
...
rays_embeddings, c2ws = model.prepare_default_rays(device, elevation=-10)
...
def process(opt: Options, path):
    ...
    with torch.no_grad():
        ...
        # 计算 3D 滤波器
        filter_3D = compute_3D_filter(gaussians, c2ws, 128, 256, 256)
        # 应用 3D 滤波
        gaussians = apply_3D_filter(gaussians, filter_3D)
        ...
        for i, img in enumerate(images):
            # if i % 15 != 0:
            #     continue
            os.makedirs(os.path.join(opt.workspace, "before_attack"), exist_ok=True)
            imageio.imwrite(os.path.join(os.path.join(opt.workspace, "before_attack"), f"{i:03d}.png"), img[0])
